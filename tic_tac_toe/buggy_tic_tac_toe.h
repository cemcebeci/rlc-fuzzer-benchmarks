...
class BuggyTicTacToeState : public State {
 public:
  BuggyTicTacToeState(std::shared_ptr<const Game> game);

  BuggyTicTacToeState(const BuggyTicTacToeState&) = default;
  BuggyTicTacToeState& operator=(const BuggyTicTacToeState&) = default;
...
// Game object.
class BuggyTicTacToeGame : public Game {
 public:
  explicit BuggyTicTacToeGame(const GameParameters& params);
  int NumDistinctActions() const override { return kNumCells; }
  std::unique_ptr<State> NewInitialState() const override {
    return std::unique_ptr<State>(new BuggyTicTacToeState(shared_from_this()));
  }
