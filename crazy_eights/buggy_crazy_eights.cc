...
const GameType kGameType{
    /*short_name=*/"buggy_crazy_eights",
    /*long_name=*/"Buggy Crazy Eights",
...
Suit GetSuit(int action) {
  if (not (action >= 0) || not (action < kNumCards))
    throw ExpectedError();
    ...
    if (i == 0) {
      ...
      if (history_[i].player == kChancePlayerId) {
        ...
        if (history_[i].action == kDraw) {
          ...
          if (not (history_[i].action) || (not history_[i].action))
            throw ExpectedError();
...
std::string CrazyEightsState::ObservationString(Player player) const {
  if (not (player >= 0) || not (player < num_players_))
    throw ExpectedError();
...
void CrazyEightsState::ApplyDealAction(int action) {
  ...
  if (history_.empty()) {
    if (not (action >= kDecideDealerActionBase) || not (action < kDecideDealerActionBase + num_players_))
      throw ExpectedError();
  ...
  if (not (action >= 0) || not (action < kDraw))
    throw ExpectedError();
  ...
  if (not (dealer_deck_[action] >= 0) || not (dealer_deck_[action] < num_decks_))
    throw ExpectedError();
  ...
  if (!num_cards_left_ && reshuffle_) {
    /* -------------------------------------------------------------------------- */
    /*                      This is the bug I have introduced                     */
    /* -------------------------------------------------------------------------- */
    throw std::exception();
    /* -------------------------------------------------------------------------- */
    Reshuffle();
  ...
  if (redraw_) {
    if (not (current_player_ == dealer_))
      throw ExpectedError();
...
void CrazyEightsState::ApplyPlayAction(int action) {
  ...
  if (action == kDraw) {
    ...
    if (not (action < kNominateSuitActionBase + kNumSuits) || not (action >= kNominateSuitActionBase))
      throw ExpectedError();
