...
class BuggyBlackjackGame;

class BuggyBlackjackState : public State {
 public:
  BuggyBlackjackState(const BuggyBlackjackState&) = default;
  BuggyBlackjackState(std::shared_ptr<const Game> game);
...
class BuggyBlackjackGame : public Game {
 public:
  explicit BuggyBlackjackGame(const GameParameters& params);

  int NumDistinctActions() const override { return 2; }
  std::unique_ptr<State> NewInitialState() const override {
    return std::unique_ptr<State>(new BuggyBlackjackState(shared_from_this()));
  }
